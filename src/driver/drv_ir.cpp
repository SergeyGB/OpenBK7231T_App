...
extern "C" commandResult_t IR_Send_Cmd(const void* context, const char* cmd, const char* args_in, int cmdFlags) {
	...
	ADDLOG_INFO(LOG_FEATURE_IR, (char*)"*** IRSend cmd [%s] arg [%s] cmdFlag %d.", cmd, args_in, cmdFlags);

	int numProtocols = sizeof(ProtocolNames) / sizeof(*ProtocolNames);
	if (!args_in) return CMD_RES_NOT_ENOUGH_ARGUMENTS;
	char args[BUFS];
	strncpy(args, args_in, BUFS - 1);
	args[BUFS - 1] = 0;
	...
	if ((*p != '-') && (*p != ' ')) {
		ADDLOG_ERROR(LOG_FEATURE_IR, (char*)"IRSend cmnd not valid [%s] not like [NEC-0-1A] or [NEC 0 1A 1]. [%s]", args, p);
		return CMD_RES_BAD_ARGUMENT;
	}

	ADDLOG_INFO(LOG_FEATURE_IR, (char*)"***(1) %s", p);
	p++;
	unsigned long RawData = strtoul(p, &p, 16);
	ADDLOG_INFO(LOG_FEATURE_IR, (char*)"***(3) %s", p);
	ADDLOG_INFO(LOG_FEATURE_IR, (char*)"***(4) %X", RawData);
	int command = RawData;
	...
	if (pIRsend) {
		...
		pIRsend->delay(100);
		ADDLOG_INFO(LOG_FEATURE_IR, (char*)"IR send %s protocol %d addr 0x%X cmd 0x%X repeats %d data 0x%X", args, (int)data.protocol, (int)data.address, (int)data.command, (int)repeats, data.decodedRawData);
		return CMD_RES_OK;
	}
	else {
